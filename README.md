...
- **Host**
    ...
    - 하나의 서버가 여러 도메인을 처리해야할 때
        ...
        *가상호스트를 통해 여러 도메인을 한번에 처리할 수 있는 서버에서 어떤 도메인으로 들어왔는지 알기위해서 Host정보가 필요하다.
        
- Location 페이지 리다이렉션
    - 웹 브라우저는 3xx 응답 결과에 Location 헤더가 있으면 해당 위치로 자동 이동
    - 201(Created) 일 때도 가능
- Allow 허용 가능한 HTTP 메서드
    - 405(Method Not Allowed) 응답을 내려줄 때 Allow에 가능한 HTTP 메서드를 넣어서 보낸다.
    - 많이 쓰지는 않음
...
- Authorization : 클라이언트 인증 정보를 서버에 전달
    - ex) Authorization : Basic xxxxxxxxxxx….
- WWW-Authentication : 리소스 접근시 필요한 인증 방법 정의
    - 401 Unauthorized 응답과 함께 사용
    - 인증이 안된 요청일 때 응답에 인증 방법을 넣어 줌
...
- 쿠키 정보는 항상 서버에 전송
    - 네트워크 트래픽 추가 유발
    - 최소한의 정보만 사용해야함(세션id, 인증 토큰)
    - *쿠키는 무조건 서버에 요청 시 보내기 때문에 이렇게 쓰지 않고 클라이언트가 정보를 가지고 있다가 보내고싶을때만 보내는 식으로 사용하고 싶다면 **웹 스토리지(localStorage, sessionStorage)**를 사용해야한다.
- 보안에 민감한 정보는 담으면 위험!

### 쿠키 - 생명주기 Expires, max-age

- Set-Cookie: **expires**=(GMT 로 되어있는 날짜)
    - 만료일이 되면 쿠키가 삭제됨
...
1. 캐시가 없을 때
    - 첫번째 요청 - 서버에 특정 사진을 요청하면 서버에서 사진을 클라이언트에 보내줌(1.1M 크기)
    - 두번째 요청 - 또 다시 같은 사진을 서버에 요청하면 또 보내줌 (1.1M 크기)
    - 데이터가 변경되지 않아도 계속 네트워크를 통해 데이터를 다운받아야함
    - 브라우저 속도가 느리다.
2. 캐시 적용
    - 첫번째 요청 - 서버에 특정 사진을 요청하면 서버에서 사진을 클라이언트에 보내줌(1.1M 크기) - **헤더에 캐시가 유효한 시간(초)를 담아서 보냄(cache-control: max-age=60)**
    - 두번째 요청 - 브라우저는 첫번째 요청에서 해당 요청을 브라우저 캐시에 담아놓는다. 그래서 두번째 요청 시 먼저 브라우저 캐시를 뒤져서 유효한 데이터가 있으면 그대로 사용 - **네트워크를 타지 않음**
    - 세번째 요청 - 캐시 시간 초과 시 서버에 다시 요청을 함

### HTTP 검증 헤더와 조건부 요청1

- 캐시 시간 초과
    - 캐시 유효시간이 초과해서 서버에 다시 요청할 시
        1. 서버에서 기존 데이터가 변경됨 → 이때는 다시 다운받음
        2. 서버에서 기존 데이터가 변경 안됨 → 이때 검증 헤더에 추가를 해보자
- 검증 헤더 추가
    1. 첫번째 요청 시 검증 헤더에 수정 날짜를 담아서 응답한다.(Last-Modified: (UTC날짜 법))
    2. 두번째 요청 시 캐시 시간이 초과 되었다면 요청할 때 If-modified-since: 수정날짜를 헤더에 담아서 보낸다.
    그러면 서버에서는 해당 요청의 데이터의 수정날짜를 검증해서 수정되지 않았다고 판단하면 응답으로 304 Not Modified으로 보낸다.(Http body가 없음)
    클라이언트는 캐시 시간이 초과된 데이터를 그냥 사용한다.
- 대부분의 웹브라우져는 이 메커니즘을 사용하고 있다.
...
- Last-Modified, If-Modified-Since 단점
    - 1초 미만의 캐시 조정이 불가능
    - 날짜 기반의 로직 사용
    - 날짜만 보기 때문에 데이터의 날짜는 수정됐어도 데이터가 수정되지 않은 경우는 캐치할 수 없음
- ETag, If-None-Match
    - ETag(Entity Tag)
    - 캐시용 데이터의 임의의 고유 버전 이름을 달아둠 -ETag: “v1.0” …
    - 데이터가 변경되면 이 이름을 바꾸기(Hash를 다시 생성)
    *hash 안에 데이터를 넣어놓으면 똑같은 데이터는 같은 hash값이 나오는 것을 이용
    - 진짜 단순하게 ETag만 보내서 같으면 유지, 다르면 다시 받기
- 캐시 제어 로직을 서버에서 완전히 관리할 수 있음
- 사용 예)
    - 애플리케이션 배포주기 마다 ETag를 모두 갱신하자
...
- Cache-Control: no-cache
    ...
    *원 서버 - 캐시 서버를 넘어서 실제 서버와 검증
    - 항상 해당 캐시가 변경이 되었는지 검증을 받고 사용하라! 라는 의미
    - Cache-Control: no-store
        - 데이터에 민감한 정보가 있으므로 저장하면 안됨(메모리에서 사용하고 최대한 빨리 삭제)

### 프록시 캐시

- 미국에 있는 유튜브 본사와 직접 네트워크 통신해서 데이터를 다운받으면 시간이 너무 오래 걸림
- 따라서 프록시 캐시를 도입한다.
    - 한국 어딘가에 프록시 캐시 서버를 두고 클라이언트가 요청 시 항상 프록시 캐시 서버를 거쳐서 통신하게 한다.
- 프록시 캐시 관련 Cache-Control 설정
...
- 확실한 캐시 무효화 응답
    ...
    - 이렇게 넣어줘야지 브라우저에서 캐시를 하지 못함
- must-revalidate
    - 프록시 캐시 서버와 원서버의 순간 네트워크가 단절 되어서 원서버에 접근 불가 시 무조건 504 Gateway timeout을 보내게 하는 설정
    - 통장 잔고같이 예민한 정보에서 사용할 수 있다.

## 참고 사항

- 참고책 - HTTP 완벽 가이드 책(RFC 2616 기준이라는 점 감안)